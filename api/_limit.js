...
export const LIMIT = {
  ...
  report:   { min: 1, day: 2  },
  naming:   { min: 1, day: 2  },
  /* ★ 2026.09.16 — med(나의 건강 밸런스)는 한 번의 측정마다 사진 6장 개별 분석 +
     통합분석 1회, 총 7건의 kind:'med' 요청을 순차로 보낸다. 분당 1회 한도로는
     2번째 사진부터 무조건 막혀 정상 사용 자체가 항상 실패했다(설계상 자기 자신을
     막는 구조). 실제 호출 횟수(7)에 여유를 더해 10으로 올린다.
     하루 한도도 기존 6회(측정 1회도 못 채우던 수준)에서 최소 하루 3번 정상 측정이
     되도록 24로 올린다(7건×3회+여유). */
  med:      { min: 10, day: 24 },
  /* ★ 2026.09.16 — med 갈래를 10으로 올려도 _all(전체 합산) 분당 6 그대로면
     med의 7번째 요청(통합분석)이 여기서 또 막힌다. 한 기능의 정상 사용(7건)이
     전체 합산보다 많은 모순을 없애기 위해 _all도 12로 올린다.
     봇 방어 취지(짧은 시간에 과도한 총 요청 차단)는 유지하면서, 정상적인 med
     사용(7건)에 여유(+5)를 더한 값 — 진짜 봇(수십~수백 회/분)은 여전히 막힌다. */
  _all:     { min: 12, day: 80 }   // 모든 갈래 합산 — 이걸 넘으면 무엇을 눌러도 잠깐 막힌다
};
